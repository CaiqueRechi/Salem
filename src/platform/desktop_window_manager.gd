...
func configure(window: Window, settings: AppSettings) -> void:
	_window = window
	...
	Logger.info(Logger.Category.WINDOW, "Desktop overlay configured.")
...
func set_mouse_passthrough(enabled: bool) -> void:
	...
	if enabled and _interactive_polygon.size() > 2:
		...
		Logger.info(Logger.Category.WINDOW, "Selective mouse passthrough enabled.")
	else:
